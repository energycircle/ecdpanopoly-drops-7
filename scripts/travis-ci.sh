...
system_install() {
  ...
  if [[ "$UPGRADE" != none ]]; then
    ...
    drush dl --yes ecdpanopoly-$UPGRADE
  fi
...
before_tests() {
  ...
  phpenv config-add drupal/profiles/ecdpanopoly/travis.php.ini
  ...
  if [[ "$UPGRADE" != none ]]; then
    ...
    cp -a ../ecdpanopoly-$UPGRADE/sites/default/* sites/default/ && drush updb --yes
    drush cc all
	drush fr-all --yes
  fi
...
after_tests() {
  ...
  # Build, commit and push to ecdpanopoly-drops-7 only if we have a tag, we are
  ...
  if [ ! -z $TRAVIS_TAG ] &&
     ...
     [ $TRAVIS_REPO_SLUG -eq "drupalprojects/ecdpanopoly" ]; then

    header Commiting and pushing changes from tag $TAG to kala-travis fork of ecdpanopoly-drops-7
    # Configure GIT
    git config --global user.email "travis@kalamuna.com"
    git config --global user.name "Travis-CI"
    # Clone ecdpanopoly-drops-7 repository and copy changes from ecdpanopoly
    cd $BUILD_TOP
    git clone https://github.com/kala-travis/ecdpanopoly-drops-7.git hotdrops
    ...
    git add --all
    git commit -m "Changes from ecdpanopoly tag [${TRAVIS_TAG}]"
    git push
